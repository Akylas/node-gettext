...
var parse_locale_data = function(locale_data, l) {
    ...
    for (domain in locale_data) {
        ...
        for (var key in data.items) {
            ...
            // remove comments, errors and references to save memory.
            delete item.comments;
            delete item.references;
            delete item.errors;
            // put the item into the right internal format to be looked up later.
            var msg_ctxt_id = item.msgctxt ? item.msgctxt + this.context_glue + item.msgid:item.msgid;
            this.data[l][domain].msgs[msg_ctxt_id] = item;
        }
    }
...
// parses a raw po file string and adds it to memory.
function parse_po (data, l, domain) {
...
Gettext.prototype = {
    // load a PO file as a string.
    loadLanguagePO: function(data, l, domain) {
        var that=this;
        parse_po.call(that, data, l, domain);
        },
    // load a JSON object.
    loadLanguageJSON: function(json,l) {
        var that=this;
        parse_locale_data.call(that,json,l);
        },
    // load a PO file from the filesystem.
    loadLanguageFile: function(file, l, callback) {
        ...
        fs.readFile(file, 'utf8', function(err, data) {
            ...
            parse_po.call(that,data,domain); // add to memory.
